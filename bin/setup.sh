...
function buildDAO() {
  ...
  popd > /dev/null
  [ ! -d "$EXTROOT/CRM/EasyBatch/DAO/" ] && mkdir -p "$EXTROOT/CRM/EasyBatch/DAO/"
  cp -f "$CIVIROOT/CRM/EasyBatch/DAO"/* "$EXTROOT/CRM/EasyBatch/DAO/"
}

##############################
function cleanup() {
  for DIR in "$XMLBUILD" "$CIVIROOT/CRM/EasyBatch/DAO" "$EXTROOT/CRM/EasyBatch/DAO/" ; do
    if [ -e "$DIR" ]; then
      rm -rf "$DIR"
    fi
  done
