...
class CRM_Batch_BAO_Batch extends CRM_Batch_DAO_Batch {
  ...
  public static function getBatchList(&$params) {
    ...
    // Join Easybatch table - This should be added via hook, but there's no support for it at the moment.
  ...
  public static function whereClause($params) {
    ...
    foreach ($fields as $field => $type) {
      ...
      if (isset($params[$field])) {
        ...
        if (in_array($field, array('batch_date', 'org_id')) && $value) {
          $table = "e";
        }
        if ($value && $type == 'Date') {
          $value = date('Ymd', strtotime($value));
          $clauses[] = "DATE_FORMAT($table.$field,'%Y%m%d')";
        }
